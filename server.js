...
app.get('/', (req, res) => {
    res.json({ status: 'running', service: 'APK Builder Pro' });
});

app.post('/build', (req, res) => {
    try {
        const { app_name, package_name, html_content, app_icon } = req.body;
        if (!app_name || !html_content) {
            return res.json({ status: 'error', message: 'اسم التطبيق ومحتوى HTML مطلوبان' });
        }
        ...
        if (app_icon && typeof app_icon === 'string' && app_icon.startsWith('data:image')) {
            try {
                const base64Data = app_icon.replace(/^data:image\/\w+;base64,/, '');
                fs.writeFileSync(`${appDir}/assets/icon.png`, Buffer.from(base64Data, 'base64'));
            } catch (err) {
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle b) {
        ...
        exec(buildCmd, { timeout: 60000 }, (err, stdout, stderr) => {
            if (err) {
                return res.json({ status: 'error', message: (stderr || err.message).slice(0, 300) });
            }
            res.json({ status: 'success', app_name, download: `/download/${safeName}.apk` });
        });
    } catch (e) {
        res.json({ status: 'error', message: 'Server exception: ' + e.message });
    }
});

let appNotifications = {};
app.post('/api/notify', (req, res) => {
    const { package_name, title, body } = req.body;
    if (!package_name || !title || !body) {
        return res.json({ status: 'error', message: 'بيانات ناقصة' });
    }
    if (!appNotifications[package_name]) appNotifications[package_name] = [];
    appNotifications[package_name].push({ title, body, time: new Date() });
    res.json({ status: 'success', message: 'تم إرسال الإشعار بنجاح' });
});

app.get('/api/notify/:package_name', (req, res) => {
    const pkg = req.params.package_name;
    res.json({ status: 'success', notifications: appNotifications[pkg] || [] });
});

app.get('/download/:file', (req, res) => {
    const fileName = req.params.file;
    const appName = fileName.replace('.apk', '');
    const filePath = `/app/apps/${appName}/${fileName}`;
    ...
    if (fs.existsSync(filePath)) {
        ...
        res.status(404).json({ status: 'error', message: 'ملف الـ APK غير موجود أو لم يتم اكتمال بنائه' });
    }
});

app.listen(process.env.PORT || 8080, () => console.log('Server running smoothly'));
