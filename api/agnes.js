export default async function handler(req, res) {
    ...
    const { action, message, prompt, image_url, size, ratio, video_id } = req.body || {};
    ...
    if (action === 'chat') {
        try {
            ...
            return res.json({ success: false, message: d.error?.message || 'خطأ في توليد الرد' });
    ...
    if (action === 'image') {
        try {
            ...
            return res.json({ success: false, message: d.error?.message || 'خطأ في توليد الصورة' });
        } catch(e) { return res.json({ success: false, message: 'خطأ في الاتصال' }); }
    }
    ...
    if (action === 'video') {
        try {
            ...
            return res.json({ success: false, message: d.error?.message || 'خطأ في إنشاء الفيديو' });
        } catch(e) { return res.json({ success: false, message: 'خطأ في الاتصال' }); }
    }
